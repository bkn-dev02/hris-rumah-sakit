...
<div class="max-w-7xl mx-auto px-6 py-8">
    ...
    <div class="mt-6 overflow-hidden rounded-lg border border-slate-200 bg-white">
        <table class="w-full text-left text-sm">
            {{-- ganti bagian <thead> dan <tbody> department dengan ini --}}
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-4 py-3">
                        <div class="flex justify-end gap-2">
                            <a href="{{ route('master.positions.edit', $position->id) }}" class="rounded-lg p-2 text-slate-400 hover:bg-blue-50 hover:text-blue-600"><i class="fa-solid fa-pen"></i></a>
                            <form method="POST" action="{{ route('master.positions.destroy', $position->id) }}" onsubmit="return confirm('Hapus position ini?')">
                                @csrf @method('DELETE')
                                <button type="submit" class="rounded-lg p-2 text-slate-400 hover:bg-red-50 hover:text-red-600"><i class="fa-solid fa-trash"></i></button>
                            </form>
                        </div>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="5" class="px-4 py-10 text-center text-sm text-slate-400">Belum ada position.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
