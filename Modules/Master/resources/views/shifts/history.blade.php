...
<div class="max-w-7xl mx-auto px-6 py-8">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-xl font-semibold text-slate-800">Riwayat Perubahan Shift — {{ $code }}</h1>
    </div>
    ...
    <div class="mt-6 overflow-hidden rounded-lg border border-slate-200 bg-white">
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr>
                    <td class="px-4 py-3 text-slate-700">{{ $version->name }}</td>
                    <td class="px-4 py-3 text-slate-600">{{ $version->start_time }} - {{ $version->end_time }}</td>
                    <td class="px-4 py-3 text-slate-500">{{ $version->effective_date->format('d M Y') }}</td>
                    <td class="px-4 py-3 text-slate-500">{{ $version->end_date?->format('d M Y') ?? '-' }}</td>
                    <td class="px-4 py-3">
