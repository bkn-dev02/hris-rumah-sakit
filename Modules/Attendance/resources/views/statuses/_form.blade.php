<div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
    <div>
        <label class="mb-1 block text-sm font-medium text-slate-700">Nama Status</label>
    ...
    <div>
        ...
        <input type="text" name="code" value="{{ old('code', $status->code ?? '') }}" placeholder="mis. HADIR, TERLAMBAT"
            class="w-full rounded-lg border border-slate-200 py-2 px-3 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
        <p class="mt-1 text-xs text-slate-400">Kode ini dipakai sistem secara internal. Hati-hati mengubah kode HADIR, TERLAMBAT, PULANG_CEPAT, LUPA_CHECKOUT — dipakai proses otomatis.</p>
        @error('code') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
    </div>

    <div>
        <label class="mb-1 block text-sm font-medium text-slate-700">Kategori</label>
        <select name="category" class="w-full rounded-lg border border-slate-200 py-2 px-3 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
    ...
    <div>
        <label class="mb-1 block text-sm font-medium text-slate-700">Tipe Penentuan</label>
        <select name="determination_type" class="w-full rounded-lg border border-slate-200 py-2 px-3 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
    ...
    <div class="flex items-center gap-2 sm:col-span-2">
        ...
        <label for="is_active" class="text-sm text-slate-700">Status aktif</label>
    </div>
</div>