...
<div class="max-w-7xl mx-auto px-6 py-8">
    ...
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-xl font-semibold text-[#1f4d3d]">Pengajuan Absensi</h1>
    </div>

    <div class="mt-6 flex gap-2 border-b border-[#dfeee1]">
        ...
        <a href="{{ route('attendance.exception-requests.index', ['status' => $value]) }}"
            class="border-b-2 px-4 py-2 text-sm font-medium {{ $approvalStatus === $value ? 'border-[#2a684f] text-[#2a684f]' : 'border-transparent text-[#2a684f] hover:text-[#2a684f]' }}">
            {{ $label }}
        </a>
        @endforeach
    </div>
    ...
    <div class="mt-4 bg-white shadow-md p-4">
        ...
        <div class="border border-gray-200 rounded-md grid grid-cols-5 gap-4 p-4 items-center text-gray-700 text-sm">
            <span class="font-medium text-[#1f4d3d]">{{ $request->employee->name }}</span>
            <span>{{ $request->work_date->format('d M Y') }}</span>
            <span><x-shared::badge variant="info" size="sm">{{ $request->status->name }}</x-shared::badge></span>
            <span class="text-[#2a684f] truncate">{{ $request->reason }}</span>
            <div class="flex justify-center">
                <a href="{{ route('attendance.exception-requests.show', $request->id) }}" class="bg-[#173f34] hover:bg-[#173f34] text-white rounded-full px-3 py-1 transition duration-200 flex items-center justify-center gap-1 hover:-transky-y-1 hover:shadow-lg">
                    <i class="fa fa-eye text-sm"></i> Detail
                </a>
            </div>
        </div>
        @empty
        <div class="border border-gray-200 rounded-md p-10 text-center text-sm text-[#2a684f]">Tidak ada pengajuan.</div>
        @endforelse
    </div>
