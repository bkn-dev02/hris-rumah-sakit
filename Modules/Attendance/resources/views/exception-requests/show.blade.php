...
<div class="max-w-7xl mx-auto px-6 py-8">
    ...
    <div class="flex items-center gap-4">
        <a href="{{ route('attendance.exception-requests.index') }}" class="border w-10 h-10 flex items-center justify-center rounded-full border-blue-100 bg-blue-900 hover:bg-blue-800 transition duration-200 translate-x-0 hover:-translate-x-1">
            <i class="fa fa-arrow-left text-md text-blue-50"></i>
        </a>
        <h1 class="text-xl font-semibold text-blue-800">Detail Pengajuan</h1>
    </div>
    ...
    <div class="mt-6 bg-white shadow-md p-6">
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div class="sm:col-span-2">
                ...
                <a href="{{ asset('storage/' . $exceptionRequest->attachment) }}" target="_blank" class="text-sm text-blue-600 hover:underline">Lihat Lampiran</a>
            </div>
            @endif
        ...
        <form id="reject-form" method="POST" action="{{ route('attendance.exception-requests.reject', $exceptionRequest->id) }}" class="mt-4 hidden">
            ...
            <textarea name="rejection_reason" rows="2" class="w-full rounded-lg border border-slate-200 py-2 px-3 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">{{ old('rejection_reason') }}</textarea>
