...
<div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
    <div>
        <label class="mb-1 block text-sm font-medium text-slate-700">Nama Role</label>
    ...
    <div class="sm:col-span-2">
        ...
        <textarea name="description" rows="2"
            class="w-full rounded-lg border border-slate-200 py-2 px-3 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">{{ old('description', $role->description ?? '') }}</textarea>
    </div>
    ...
    <div class="flex items-center gap-2 sm:col-span-2">
        ...
        <label for="is_active" class="text-sm text-slate-700">Role aktif</label>
    </div>
</div>

<div class="mt-6">
    <label class="mb-2 block text-sm font-medium text-slate-700">Permission</label>

    <div class="space-y-4 rounded-lg border border-slate-200 p-4">
        @foreach($permissionGroups as $group => $permissions)
        <div>
            <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">{{ ucfirst($group) }}</p>
