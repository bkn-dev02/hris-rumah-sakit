<div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
    ...
    <div class="sm:col-span-2">
        ...
        <input type="text" name="code" value="{{ old('code', $permission->code ?? '') }}" placeholder="mis. employees.view"
            class="w-full rounded-lg border border-slate-200 py-2 px-3 text-sm focus:border-[#2a684f] focus:outline-none focus:ring-1 focus:ring-[#dfeee1]">
        <p class="mt-1 text-xs text-slate-400">Format: resource.aksi, huruf kecil. Kode ini dipakai langsung di route sebagai middleware â€” ubah dengan hati-hati kalau permission ini sudah dipakai.</p>
        @error('code') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
    </div>

    <div class="sm:col-span-2">
        <label class="mb-1 block text-sm font-medium text-slate-700">Deskripsi</label>
        <textarea name="description" rows="2" class="w-full rounded-lg border border-slate-200 py-2 px-3 text-sm focus:border-[#2a684f] focus:outline-none focus:ring-1 focus:ring-[#dfeee1]">{{ old('description', $permission->description ?? '') }}</textarea>
